//
//  SecondDrawClickerBarGraphView.swift
//  HALTeacher
//
//  Created by Toshiki Higaki on 2015/10/09.
//  Copyright (c) 2015年 sotuken. All rights reserved.
//

import UIKit

@objc protocol SecondDrawClickerBarGraphViewDelegate {
    optional func pushBarGraph(markNumber: Int)
}

class SecondDrawClickerBarGraphView: UIView {
    
    var delegate: SecondDrawClickerBarGraphViewDelegate!
    ...
    */
    ...
    override func drawRect(rect: CGRect) {
        for(var i=0 ;i<ClickerInfo.clickerInfoInstance.buttonTapCount.count ;i++){
            ...
            // 高度な計算
            // 個別
            let oneWidth = SecondWindow.secondWindowInstance.secondScreenWidth! / CGFloat(ClickerInfo.clickerInfoInstance.buttonTapCount.count)
            let barWidth = oneWidth / 2
            let initBarPointX = CGFloat(oneWidth / 4) * CGFloat(i)
            
            let initBarPointY = SecondWindow.secondWindowInstance.secondScreenHeight! - 200
            let barMaxHeight = self.bounds.height
            
            CGContextFillRect(cgContext, CGRect(x: Int(initBarPointX), y: Int(initBarPointY), width: Int(barWidth), height: Int(percentage) / 100 * Int(barMaxHeight)))
            barView = UIView(frame: CGRect(x: Int(initBarPointX), y: Int(initBarPointY), width: Int(barWidth), height: Int(percentage) / 100 * Int(barMaxHeight)))
//            
//                        //割合0.0 = 押してない
//                        if(percentage != 0.0){
